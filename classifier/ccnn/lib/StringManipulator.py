...
class StringManipulator:
	...
	def remove_stopwords(cls, txt):
		"""
			...
			Returns:
				f_text(str): filtered string
		"""
		filtered_list = np.array([word for word in txt.split(" ") if word not in cls._stop])
	...
	def normalize_text(cls, text, remove_stopword=True):
		...
		if (remove_stopword):
			normalized_text = [word for word in normalized_list.split(" ") if word not in cls._stop]
			normalized_text = " ".join(normalized_text)
		else:
			normalized_text = [word for word in normalized_list.split(" ")]
			normalized_text = " ".join(normalized_text)
