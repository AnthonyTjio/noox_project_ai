...
from .CCNNModel import CCNNModel
from .CCNNPreprocessor import CCNNPreprocessor
...
class CCNNProcessor:
	...
	@classmethod
	def initial_train(cls, training_dir, content_rows=[1,2], label_row=3, ratio=0.6):
		with tf.Graph().as_default():
			...
			data = CCNNPreprocessor.normalize_content_data(data)
			input_data, label_data = CCNNPreprocessor.convert_dataset(data)

			training_indices, test_indices = CCNNPreprocessor.shuffleData(data, ratio=ratio)
	...
	def training(cls, training_dir="./small_data.csv", content_rows=[1,2], label_row=3):
		...
		content_data = CCNNPreprocessor.normalize_content_data(content_data)
		content_data = CCNNPreprocessor.merge_content_data(content_data)
		# content_vector = CCNNPreprocessor.convert_content_data_to_vector(content_data, cls.alphabet)
